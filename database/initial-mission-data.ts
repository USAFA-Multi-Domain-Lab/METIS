// -- initial --

export let demoMissionData = {
  name: 'METIS > ASCOT 7 DEMO',
  versionNumber: 1,
  seed: '6499bf5b8214651c898d125f',
  initialResources: 100,
  live: true,
  nodeStructure: {
    '882d6985-a05c-4f36-b66b-92de27d34552': {
      '405b84dc-a653-47ed-8d88-aef3177a99b2': {},
      '315605d0-3210-4644-8903-d254151769e2': {},
      '51f256b3-614c-4f9a-80f3-3d020a758cd7': {},
      '2d69024c-9cc4-40bd-ab84-1d7761abe810': {},
      'd4bf672b-1490-4246-92c5-69e31d2714b5': {
        '6185ea46-de8b-4308-aec3-e257313563af': {
          'e25eb328-d98a-4215-a1a2-e854137e9209': {
            '152811b3-e125-4cae-b4e4-06a5842b403c': {
              'b6220969-160d-4339-8a96-b5c632422ffa': {},
              '0d0ad493-346c-4096-80e3-f8bc7b21f535': {},
              'a8bbd3c1-b050-40cc-932a-5b642e9e5d37': {},
              '71eb8dd1-1104-481b-baf0-a4850c58ebc7': {},
              '7344d4d7-2a9b-4b99-ad2a-4dd353a3bd49': {
                'c3a94935-578b-42a1-9fc1-7878fe24fb6d': {},
                'ae6b07fb-9f36-4eeb-9dbd-e579e8eb7f5d': {},
                '60acf5d1-3467-462c-816c-bcb2e5c2b8e0': {
                  '26f3833d-45cf-4860-ac97-edd8ef877d3c': {
                    '3bca4ca3-4a5b-4518-bc7a-0f2c8d9b1999': {},
                    'a6a31b1c-7be6-4a71-81c1-1ffa88ed3711': {},
                    'e398028a-1f87-4d58-bf69-b949ef176cf6': {
                      'd04326e3-491b-479a-9f22-d4f49703f8de': {},
                    },
                  },
                },
              },
              'c3b45b64-160d-4fca-a0bf-10fc8740ea21': {},
            },
            '1c97200f-69d6-4546-b99e-6cd0a11ae0a4': {},
          },
          'cb7c666e-4ac3-4c6c-8dcd-c8c4d383ce65': {},
          'e1ef9397-f480-4f06-8c0b-afcf1ace7f69': {},
          '096fa5e6-d25a-4b86-a618-a8ab854d9b12': {},
          '36ed93ea-a7ef-445e-b1a2-d068681ebca4': {},
          '05b4b9e6-2f46-4ce0-adf4-d1d846c650da': {},
        },
      },
      'afa6923d-875f-4dbe-b724-3c908deb853b': {},
      '339e1633-34cc-488f-abee-61ec1f21f0dd': {},
    },
  },
  nodeData: [
    {
      nodeID: '882d6985-a05c-4f36-b66b-92de27d34552',
      name: 'Select Target Region',
      color: '#65eb59',
      description: 'Select Combatant Command',
      preExecutionText:
        'SYS MESSAGE: Select the unified combatant commands in which to conduct operations.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [
        {
          actionID: 'af3fc223-44a6-4072-bc61-2d962feaa0df',
          name: 'New Action',
          description: 'Enter your description here.',
          processTime: 5000,
          successChance: 0.5,
          resourceCost: 1,
          postExecutionSuccessText:
            'Enter your successful post-execution message here.',
          postExecutionFailureText:
            'Enter your failed post-execution message here.',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '405b84dc-a653-47ed-8d88-aef3177a99b2',
      name: 'CENTCOM',
      color: '#52b1ff',
      description: 'U.S. Central Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '315605d0-3210-4644-8903-d254151769e2',
      name: 'AFRICOM',
      color: '#52b1ff',
      description: 'U.S. Africa Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '51f256b3-614c-4f9a-80f3-3d020a758cd7',
      name: 'EUCOM',
      color: '#52b1ff',
      description: 'U.S. European Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '2d69024c-9cc4-40bd-ab84-1d7761abe810',
      name: 'NORTHCOM',
      color: '#52b1ff',
      description: 'U.S. Northern Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: 'd4bf672b-1490-4246-92c5-69e31d2714b5',
      name: 'INDOPACOM',
      color: '#52b1ff',
      description: 'U.S. Indo-Pacific Command',
      preExecutionText:
        "*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant command's cyberspace effects services interface system.\n\nMISSION: U.S. Indo-Pacific Command will implement a combat credible deterrence strategy capable of denying our adversaries sustained air and sea dominance by focusing on posturing the Joint Force to win before fighting, while being ready to fight and win, if required.",
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: 'afa6923d-875f-4dbe-b724-3c908deb853b',
      name: 'SOUTHCOM',
      color: '#52b1ff',
      description: 'U.S. Southern Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '339e1633-34cc-488f-abee-61ec1f21f0dd',
      name: 'SPACECOM',
      color: '#52b1ff',
      description: 'U.S. Space Command',
      preExecutionText:
        '*** CONNECTION ESTABLISHED ***\nSYS MESSAGE: Welcome to the unified combatant commands cyberspace effects services interface system.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '6185ea46-de8b-4308-aec3-e257313563af',
      name: 'Select Domain',
      color: '#eb5fb2',
      description: 'Select Domain of Operations',
      preExecutionText: 'SYS MESSAGE: Select domain of operations.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: 'e25eb328-d98a-4215-a1a2-e854137e9209',
      name: 'Space',
      color: '#cd328e',
      description: 'Target Space Domain',
      preExecutionText: 'SYS MESSAGE: Select target region.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: 'cb7c666e-4ac3-4c6c-8dcd-c8c4d383ce65',
      name: 'Cyberspace',
      color: '#cd328e',
      description: 'Target Cyberspace Domain',
      preExecutionText:
        'SYS MESSAGE: Selected domain assets currently available for cyberspace effects targets has been compiled.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: 'e1ef9397-f480-4f06-8c0b-afcf1ace7f69',
      name: 'Air',
      color: '#cd328e',
      description: 'Target Air Domain',
      preExecutionText:
        'SYS MESSAGE: Selected domain assets currently available for cyberspace effects targets has been compiled.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '096fa5e6-d25a-4b86-a618-a8ab854d9b12',
      name: 'Land',
      color: '#cd328e',
      description: 'Target Land Domain',
      preExecutionText:
        'SYS MESSAGE: Selected domain assets currently available for cyberspace effects targets has been compiled.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '36ed93ea-a7ef-445e-b1a2-d068681ebca4',
      name: 'Sea',
      color: '#cd328e',
      description: 'Target Sea Domain',
      preExecutionText:
        'SYS MESSAGE: Selected domain assets currently available for cyberspace effects targets has been compiled.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '05b4b9e6-2f46-4ce0-adf4-d1d846c650da',
      name: 'Sub-Surface',
      color: '#cd328e',
      description: 'Target Sub-Surface Domain',
      preExecutionText:
        'SYS MESSAGE: Selected domain assets currently available for cyberspace effects targets has been compiled.',
      depthPadding: 0,
      executable: false,
      device: false,
      actions: [],
    },
    {
      nodeID: '152811b3-e125-4cae-b4e4-06a5842b403c',
      name: 'China',
      color: '#f5e677',
      description: 'Select Target Region',
      preExecutionText:
        '*** WARNING: ACCESS AND OPERATIONS CONDUCTED BEYOND THIS POINT REQUIRES COMBATANT COMMAND LEAVE AUTHORIZATION. ***',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: 'ae76adbd-b74a-44c1-8ff9-30ae8d81d87e',
          name: 'CONFIRM AUTHORIZATION',
          description: 'ACCESS WARNING',
          processTime: 10000,
          successChance: 1,
          resourceCost: 0,
          postExecutionSuccessText:
            "*** RUNNING COMPILE SCRIPT. . . . . . [] ***\n\nSYS MESSAGE: Compiling list of Xi’an Satellite Control Center subordinate ground control stations (GCS).\n系统消息：正在整理西安卫星控制中心下属地面控制站（GCS）列表。\nXìtǒng xiāoxī: Zhèngzài zhěnglǐ xī'ān wèixīng kòngzhì zhōngxīn xiàshǔ dìmiàn kòngzhì zhàn (GCS) lièbiǎo.",
          postExecutionFailureText: 'Error: 1025845',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '1c97200f-69d6-4546-b99e-6cd0a11ae0a4',
      name: 'North Korea',
      color: '#f5e677',
      description: 'Select Target Region',
      preExecutionText:
        '*** WARNING: Access beyond this point requires combatant command leave authorization. ***',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '6ce8497c-9f49-48a6-95d5-1ca4ab22926f',
          name: 'CONFIRM AUTHORIZATION',
          description: 'ACCESS WARNING',
          processTime: 5000,
          successChance: 1,
          resourceCost: 0,
          postExecutionSuccessText:
            '*** RUNNING COMPILE SCRIPT. . . . . . [] ***\nSYS MESSAGE: List of available access points has been generated.',
          postExecutionFailureText: 'Error: 1025845',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'b6220969-160d-4339-8a96-b5c632422ffa',
      name: 'Nanning Station [南宁站] ',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '4dc45bcc-40f5-4e0b-a49c-fcc1ecf67a91',
          name: 'Run Brute Force Entry Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// BRUTE FORCE CRACK SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '0d0ad493-346c-4096-80e3-f8bc7b21f535',
      name: 'Lingshui Station [陵水测控站]',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '733a8fef-0877-4719-a71e-53daee435f64',
          name: 'Run Brute Force Entry Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// BRUTE FORCE CRACK SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'a8bbd3c1-b050-40cc-932a-5b642e9e5d37',
      name: 'Menghai Station [勐海测控站]',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: 'b9562a57-bb0f-4bb2-8000-33c2db9c26ec',
          name: 'Run Brute Force Entry Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// BRUTE FORCE CRACK SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '71eb8dd1-1104-481b-baf0-a4850c58ebc7',
      name: 'Minxi Station [闽西测控站]',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '2a89ea42-76a6-4b20-a998-08659a4c7639',
          name: 'Run Brute Force Entry Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// BRUTE FORCE CRACK SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '7344d4d7-2a9b-4b99-ad2a-4dd353a3bd49',
      name: 'Changchun Station [长春测控站] ',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText:
        'Now connected to Changchun Station monitor/command systems.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '81c1997e-a0f3-4f74-bffc-f0d2f3aac873',
          name: 'Run Access Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// ACCESS SCRIPT SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.\n/// 访问脚本成功 ///\n系统消息：当前连接的地面资产的列表。\n/// Fǎngwèn jiǎoběn chénggōng///\nxìtǒng xiāoxī: Dāngqián liánjiē dì dìmiàn zīchǎn dì lièbiǎo.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'c3b45b64-160d-4fca-a0bf-10fc8740ea21',
      name: 'Qingdao Station [青岛测控站]',
      color: '#d0bf3b',
      description: 'Select Ground Control Station',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: 'efc68198-4909-4134-9fa4-fd2a948b82de',
          name: 'Run Brute Force Entry Script',
          description: 'Gain access to this system.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 10,
          postExecutionSuccessText:
            '/// BRUTE FORCE CRACK SUCCESSFUL ///\nSYS MESSAGE: List compiled of currently connected terrestrial based assets.',
          postExecutionFailureText:
            'Error: 1025845 *** NETWORK ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'c3a94935-578b-42a1-9fc1-7878fe24fb6d',
      name: 'YAOGAN-23 (SAR)',
      color: '#52b1ff',
      description: 'JB-7-4',
      preExecutionText:
        'Satellite: YAOGAN-23 (SAR) Two Line Element Set (TLE): \n1 40305U 14071A   23186.44045114  .00010456  00000-0  35544-3 0  9998\n2 40305  97.6540 150.2916 0001821 103.7780 256.3663 15.30655096478928',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '6bdc0edd-ac17-41f4-8e32-57283689a83f',
          name: 'New Action',
          description: 'Enter your description here.',
          processTime: 5000,
          successChance: 0.5,
          resourceCost: 1,
          postExecutionSuccessText:
            'Enter your successful post-execution message here.',
          postExecutionFailureText:
            'Enter your failed post-execution message here.',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'ae6b07fb-9f36-4eeb-9dbd-e579e8eb7f5d',
      name: 'YAOGAN-35 (ELINT)',
      color: '#52b1ff',
      description: 'Unknown',
      preExecutionText:
        'Satellite: YAOGAN-35 (ELINT) - Two Line Element Set (TLE): \n1 53522U 22100A   23185.93674371  .00000372  00000-0  15955-4 0  9992\n2 53522  35.0016 357.2127 0015138 230.2255 129.7127 15.25075460 48723',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: 'aed7e61c-7c86-4e13-b1b2-0900be1879f1',
          name: 'New Action',
          description: 'Enter your description here.',
          processTime: 5000,
          successChance: 0.5,
          resourceCost: 1,
          postExecutionSuccessText:
            'Enter your successful post-execution message here.',
          postExecutionFailureText:
            'Enter your failed post-execution message here.',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '60acf5d1-3467-462c-816c-bcb2e5c2b8e0',
      name: 'YAOGAN-36 (Data)',
      color: '#52b1ff',
      description: 'Unknown',
      preExecutionText:
        'YAOGAN-36 (Data) - Two Line Element Set (TLE): \n1 54372U 22160A   23186.55133669  .00108949  00000-0  42295-2 0  9995\n2 54372  34.9942 281.0301 0008014  20.3153 357.7082 15.24733845 33602',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '38f455e4-2e75-4ead-9cbd-c8c0c9d2a3b2',
          name: 'Exploit Data',
          description: 'Pull asset connection data.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 1,
          postExecutionSuccessText:
            'Currently Connected: Chengdu GJ-2 (Wing Loong 2) - SPEED: 320 km/h - ALTITUDE: 28k MSL - LOCATION: 43° 30.1647′ N, 141° 05.6799′ E\n目前连接：成都 GJ-2（翼龙 2） - 速度：320 km/h - 海拔：28k MSL - 位置：43°30.1647′N，141°05.6799′E\nMùqián liánjiē: Chéngdū GJ-2(yì lóng 2) - sùdù:320 Km/h - hǎibá:28K MSL - wèizhì:43°30.1647′N,141°05.6799′E',
          postExecutionFailureText: 'Error: 10295456 *** ACCESS BLOCKED ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '26f3833d-45cf-4860-ac97-edd8ef877d3c',
      name: 'Chengdu GJ-2 (UAV)',
      color: '#b839ff',
      description: 'Wing Loong 2 (UAV, China)',
      preExecutionText:
        'Last Reported Data: Chengdu GJ-2 (Remotely Piloted Aircraft) - Current SPEED: 320 km/h - Current ALTITUDE: 28k MSL - Current LOCATION: 43° 30.1647′ N, 141° 05.6799′ E',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: '4fe91792-7226-4d31-a4cc-a8448ef08e20',
          name: 'Access Control Systems',
          description: 'Access UAV On-board Systems.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 5,
          postExecutionSuccessText:
            '/// Access gained to UAV on-board systems. ///\n/// 获得无人机机载系统的访问权限。 ///\n/// Huòdé wú rén jī jī zǎi xìtǒng de fǎngwèn quánxiàn. ///',
          postExecutionFailureText:
            'Error: 10295456 *** Air Vehicle has rejected command. ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: '3bca4ca3-4a5b-4518-bc7a-0f2c8d9b1999',
      name: 'Flight Controls',
      color: '#b839ff',
      description: 'Access air vehicle flight controls.',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: true,
      actions: [
        {
          actionID: 'fa1b2d96-ae4b-47e3-b9a2-2da2c5097a99',
          name: 'Update Flight Command',
          description: 'Update Speed Control',
          processTime: 10000,
          successChance: 1,
          resourceCost: 5,
          postExecutionSuccessText:
            '/// Commanded Speed: 0 km/h (Confirmed) ///',
          postExecutionFailureText:
            'Error: 10295456 *** Air Vehicle has rejected command. ***',
          scripts: [],
        },
        {
          actionID: 'a630016c-b075-47a8-8ec0-5768654ac451',
          name: 'New Action',
          description: 'Enter your description here.',
          processTime: 5000,
          successChance: 0.5,
          resourceCost: 1,
          postExecutionSuccessText:
            'Enter your successful post-execution message here.',
          postExecutionFailureText:
            'Enter your failed post-execution message here.',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'a6a31b1c-7be6-4a71-81c1-1ffa88ed3711',
      name: 'Access Controls',
      color: '#b839ff',
      description: 'Access air vehicle system lock-out.',
      preExecutionText: 'Node has not been executed.',
      depthPadding: 0,
      executable: true,
      device: true,
      actions: [
        {
          actionID: 'd937a941-948f-4ff3-9cde-020e6f27a524',
          name: 'Lock System Controls',
          description: 'Lock UAV Access Controls',
          processTime: 10000,
          successChance: 1,
          resourceCost: 1,
          postExecutionSuccessText:
            '/// Air Vehicle Controls *LOCKED*, Executing Loss-of-Link Procedures. (Confirmed) ///',
          postExecutionFailureText:
            'Error: 10295456 *** Air Vehicle has rejected command. ***',
          scripts: [],
        },
        {
          actionID: 'aaaaf89b-2bb0-458a-9268-a6a8adbd7d6d',
          name: 'New Action',
          description: 'Enter your description here.',
          processTime: 5000,
          successChance: 0.5,
          resourceCost: 1,
          postExecutionSuccessText:
            'Enter your successful post-execution message here.',
          postExecutionFailureText:
            'Enter your failed post-execution message here.',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'e398028a-1f87-4d58-bf69-b949ef176cf6',
      name: 'Emergency Controls',
      color: '#b839ff',
      description: 'Execute Emergency Loss-of-Link.',
      preExecutionText: 'UAV will execute loss-of-link protocol.',
      depthPadding: 0,
      executable: true,
      device: false,
      actions: [
        {
          actionID: 'e2d7772c-f2bc-4085-9c8b-be158cc8c4b3',
          name: 'Force Loss-of-Link',
          description: 'UAV will execute loss-of-link protocol.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 1,
          postExecutionSuccessText:
            '/// DESTRUCTION WARNING ///\nSYS MESSAGE: Forcing loss-of-link protocol will result in UAV self-destruction.\n*** CONFIRM ACTION ***\n/// 破坏警告 ///\n系统消息：强制使用链路丢失协议将导致无人机自毁。\n*** 确认操作 ***\n/// Pòhuài jǐnggào///\nxìtǒng xiāoxī: Qiángzhì shǐyòng liàn lù diūshī xiéyì jiāng dǎozhì wú rén jī zì huǐ.\n*** Quèrèn cāozuò***',
          postExecutionFailureText:
            'Error: 10295456 *** Air Vehicle has rejected command. ***',
          scripts: [],
        },
      ],
    },
    {
      nodeID: 'd04326e3-491b-479a-9f22-d4f49703f8de',
      name: '*** WARNING ***',
      color: '#f9484f',
      description: '*** CONFIRM ACTION ***',
      preExecutionText: '*** CONFIRM ACTION ***',
      depthPadding: 0,
      executable: true,
      device: true,
      actions: [
        {
          actionID: '39f2c434-4f8b-42df-87e0-83070034e89b',
          name: 'Send Self-destruction Command',
          description: 'Enter your description here.',
          processTime: 10000,
          successChance: 1,
          resourceCost: 1,
          postExecutionSuccessText:
            '/// UAV HAS RECIEVED SELF-DESTRUCTION COMMAND ///\nSYS MESSAGE: Connection to UAV (Chengdu GJ-2) has terminated.\n/// 无人机已收到自毁命令 ///\n系统消息：与无人机（成都 GJ-2）的连接已终止。\n/// Wú rén jī yǐ shōu dào zì huǐ mìnglìng///\nxìtǒng xiāoxī: Yǔ wú rén jī (chéngdū GJ-2) de liánjiē yǐ zhōngzhǐ.',
          postExecutionFailureText:
            'Error: 10295456 *** Air Vehicle has rejected command. ***',
          scripts: [],
        },
      ],
    },
  ],
  introMessage: 'Enter your overview message here.',
}
