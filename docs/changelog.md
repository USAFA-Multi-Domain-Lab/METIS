...
- A standalone session mode has been added. Every participant is placed in their own realm containing only one selected force, so everyone in a group can work through the same mission at their own pace within one session without affecting each other.
- Observers and managers can switch between realms while a session is running, using a searchable list that can be navigated by keyboard. This allows them to follow any participant's progress through a standalone session.
- The lobby page has been greatly overhauled with a new, more compact design. The session-configuration page responsible for updating a configuration after launch has now been consolidated with the lobby page. Changes made to the configuration on the lobby page will now auto-save, rather than requiring a save button to be clicked.
- The launch page design has been adjusted.
- App design in general has been tweaked, with elements such as the navigation and various forms now displaying more compactly, allowing more content to be displayed in a window at once.
...
- Play testing missions will now immediately navigate to the session page, rather than first navigating to a session configuration menu.
...
- A mission-wide option has been added to the issues panel for updating outdated effects in bulk.
- Effect-broadcast support has been implemented. Now, one effect can manipulate a wide and diverse range of components in a mission, without the need to create many similar effects. A new form-field type has been added and implemented across the mission builder, allowing mission designers to quickly and easily select the scope of their effects.
...
- Selecting a component on the mission page now brings the inspector tab forward automatically, so the selected component's properties appear without having to also switch tabs manually.
- Effects are now created within a secondary panel view instead of a modal placed over the mission map.
- Parameter dependencies now work in a chain, meaning, if parameter A depends on parameter B, and parameter B depends on parameter C, then parameter A also depends on parameter C without it being explictly written in the schema.
- Dropdown parameter options can now define tooltip descriptions.
- The METIS target environment has been updated to use the new argument format and includes migration scripts to convert existing effects.
