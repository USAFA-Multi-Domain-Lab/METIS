# METIS Documentation Index

Welcome to the official documentation for METIS. This index serves as the central point of reference for setup instructions, API guides, developer documentation, and release notes.

---

## 📦 Setup Instructions

- [Debian Setup](./setup/debian.md) - Detailed setup for Debian environments.
- [Windows Setup](./setup/windows.md) - Detailed setup for Windows environments.

---

## 🎓 Getting Started with METIS

For a comprehensive walkthrough of METIS features and functionality, please watch our [video guide](https://youtu.be/4rcTbKRUOEA?si=XPy1z3Y4KJ0Fg5E1).

---

## 🔌 API Documentation

- [Overview](./api/overview.md)
- [Missions](./api/missions.md)
- [Users](./api/users.md)
- [Sessions](./api/sessions.md)
- [Target Environments](./api/target-environments.md)
- [Files](./api/files.md)
- [Logins](./api/logins.md)
- [Info](./api/info.md)

---

## 🛠 Developer Documentation

- [Target Environment Integration](./developer/target-environment-integration.md) - Guide on creating target environment plugins.
- [Architecture](./devs/architecture.md) - Understand the system architecture.
- [Style Guide](./developer/style-guide.md) - Coding standards and best practices.

---

## 📝 Changelog

- [View Changelog](./changelog.md)
