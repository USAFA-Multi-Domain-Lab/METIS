...
import MissionAction, { IMissionActionJSON } from 'metis/missions/old/actions'
import { IMissionMappable } from 'metis/missions/old'
...
export default class MissionNode implements IMissionMappable {
  ...
  selectedAction: MissionAction | null
  private _executingAction: MissionAction | null
  private _executing: boolean
  // ! TEMPORARILY PUBLIC FOR TESTING
  public _lastExecutedAction: MissionAction | null
  // ! TEMPORARILY PUBLIC FOR TESTING
  ...
  static default_actions: Array<IMissionActionJSON> = []
  static default_mapX: number = 0
  static default_mapY: number = 0
  static default_isOpen: boolean = false

  static createDefaultAction(node: MissionNode): MissionAction {
   ...
   */
  public get lastExecutedAction(): MissionAction | null {
  ...
  get executingAction(): MissionAction | null {
   ...
   * @param {Array<IMissionActionJSON>} actionJSON The JSON for the actions that this node will have.
  ...
  public constructor(
    ...
    executable: boolean,
    device: boolean,
    actionJSON: Array<IMissionActionJSON>,
  ...
  public toJSON(): IMissionNodeJSON {
    return {
      ...
      device: this.device,
      actions: this.actions.map((action: MissionAction) => action.toJSON()),
      isOpen: this.isOpen,
    }
  }
  ...
  parseActionJSON(actionJSON: Array<IMissionActionJSON>): void {
    let actions = []

    for (let action of actionJSON) {
      let actionObject: MissionAction = new MissionAction(
        this,
        action.actionID,
        action.name,
        action.description,
  ...
  handleActionExecutionStart(action: MissionAction): void {
  ...
  handleActionExecutionEnd(success: boolean): void {
    let executingAction: MissionAction | null = this.executingAction
