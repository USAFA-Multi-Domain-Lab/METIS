...
export default function NumberTargetParameter({
  ...
  const [requiredValue, setRequiredValue] = useState<number>(() => {
    ...
    if (arg.type === 'number' && arg.required) {
      return targetArguments[arg._id] ?? arg.default
    } else {
      return 0
    }
  })
  const [optionalValue, setOptionalValue] = useState<number | null>(
    targetArguments[arg._id] ?? null,
  )
  ...
  usePostInitEffect(() => {
    ...
    if (arg.required) {
      setTargetArguments((prev) => ({ ...prev, [arg._id]: requiredValue }))
    }
    // Or, if the argument is optional...
      ...
      if (optionalValue !== null) {
        setTargetArguments((prev) => ({ ...prev, [arg._id]: optionalValue }))
      }
